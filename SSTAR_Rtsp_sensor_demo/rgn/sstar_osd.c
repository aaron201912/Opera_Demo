...
void _gfx_sink_surface(void* pdate, const char  *name)
{
    ...
    unsigned char *pdata = buf->vaddr_0;
...
void osd_fill_canvas(void* pdate, int rgba, int width, int height, int xpos, int ypos)
{
    ...
    pixel_u *dst = (pixel_u *)buf->vaddr_0 + ypos*canvas_line_pixels + xpos;
...
void test_fill_nv12(void* pdate)
{
    ...
    char* u;
    char* v;
    u = (char*)buf->vaddr_0 + (buf->height * buf->width);
    v = (char*)buf->vaddr_0 + (buf->height * buf->width) + 1;
    fill_tiles_osd_yuv_planar((char*)buf->vaddr_0, u, v, buf->width, buf->height, buf->width);
