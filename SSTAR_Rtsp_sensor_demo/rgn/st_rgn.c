...
MI_S32 ST_OSD_Init(MI_S32 s32ChipId)
{
    ...
    if (g_bInit == FALSE)
    {
        ...
        ExecFunc(MI_RGN_Init(s32ChipId, &g_stPaletteTable), MI_RGN_OK);
    }
...
MI_S32 ST_OSD_Deinit(MI_S32 s32ChipId)
{
    ...
    if (g_bInit == TRUE)
    {
        ...
        ExecFunc(MI_RGN_DeInit(s32ChipId), MI_RGN_OK);
    }
    pthread_mutex_unlock(&g_rgnOsd_mutex);
...
MI_S32 sstar_init_rgn(buffer_object_t *buf_obj)
{
    ...
    *************************************************/
    ...
    {
        stRegion.stOsdInitParam.stSize.u32Width = buf_obj->vdec_info.v_out_height;
        stRegion.stOsdInitParam.stSize.u32Height = buf_obj->vdec_info.v_out_width;
    }else
    {
        stRegion.stOsdInitParam.stSize.u32Width = buf_obj->vdec_info.v_out_width;
        stRegion.stOsdInitParam.stSize.u32Height = buf_obj->vdec_info.v_out_height;  
    }

    ST_OSD_Create(0, _g_stRgnHandle, &stRegion);
    stRgnChnPort.eModId = buf_obj->rgn_chn_port_info.eModId;
    stRgnChnPort.s32DevId = buf_obj->rgn_chn_port_info.u32DevId;
    stRgnChnPort.s32ChnId = buf_obj->rgn_chn_port_info.u32ChnId;
    stRgnChnPort.s32PortId = buf_obj->rgn_chn_port_info.u32PortId;
