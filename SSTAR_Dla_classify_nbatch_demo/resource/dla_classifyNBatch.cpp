...
static void IPU_Free_Inputtensor_Memory(MI_IPU_SubNet_InputOutputDesc_t desc, MI_IPU_BatchInvokeParam_t stInvokeParam,MI_BOOL bBatchOneBuf)
{
    if(bBatchOneBuf)
    {
        for (MI_U32 idxInputNum = 0; idxInputNum < desc.u32InputTensorCount; idxInputNum++)
        {
            IPU_TensorFree(&stInvokeParam.astArrayTensors[idxInputNum], IPU_CalcTensorSize(desc.astMI_InputTensorDescs[idxInputNum]) * stInvokeParam.u32BatchN);
...
static void IPU_Free_Outputtensor_Memory(MI_IPU_SubNet_InputOutputDesc_t desc, MI_IPU_BatchInvokeParam_t stInvokeParam,MI_U32 totalInputNumber,MI_BOOL bBatchOneBuf)
{
    if(bBatchOneBuf)
    {
        for (MI_U32 idxOutputNum = 0; idxOutputNum < desc.u32OutputTensorCount; idxOutputNum++)
        {
            IPU_TensorFree(&stInvokeParam.astArrayTensors[idxOutputNum + totalInputNumber], IPU_CalcTensorSize(desc.astMI_OutputTensorDescs[idxOutputNum]) * stInvokeParam.u32BatchN);
        }
    }
    else
    {
...
int main(int argc,char *argv[])
{
    if ( argc < 6 )
    {
        ...
        << "<picture> " << "<labels> "<< "<model intput_format:RGB or BGR>"<< "<batch number: 1~10> "<<std::endl;
        exit(0);
    } else {
         ...
         std::cout<<"batch number:"<<argv[5]<<std::endl;
    }
    ...
    //9. specify which IPU to use and batch number
    ...
    stInvokeParam.u32IpuAffinity = IPU_DEV_0|IPU_DEV_1;
    stInvokeParam.u32BatchN = batchNum;

    MI_U32 totalInputNumber=0;
    //10. Alloc in/out tensors memory according to tensor num and size and batch number.
    if(OfflineModelInfo.eBatchMode == E_IPU_BATCH_ONE_BUF_MODE)
    {
        bBatchOneBuf = TRUE;
        cout<<"model batch mode: one_buf"<<endl;
    }
    else
    {
        bBatchOneBuf = FALSE;
        cout<<"model batch mode: n_buf"<<endl;
    }

    if(bBatchOneBuf)
    {
        //batch_one_buf mode: All batch buffers of each input/output is combined together.
        //layout example: u32BatchN = 3; u32InputTensorCount = 2; u32OutputTensorCount = 2;
        // stInvokeParam.astArrayTensors[0]  for input0  batch0 + batch1 + batch2
        // stInvokeParam.astArrayTensors[1]  for input1  batch0 + batch1 + batch2
        // stInvokeParam.astArrayTensors[2]  for output0 batch0 + batch1 + batch2
        // stInvokeParam.astArrayTensors[3]  for output1 batch0 + batch1 + batch2
        for (MI_U32 idxInputNum = 0; idxInputNum < desc.u32InputTensorCount; idxInputNum++)
        {
            if (MI_SUCCESS != IPU_TensorMalloc(&stInvokeParam.astArrayTensors[idxInputNum], IPU_CalcTensorSize(desc.astMI_InputTensorDescs[idxInputNum]) * stInvokeParam.u32BatchN))
            {
                delete pu8ImageData;
            ...
            for (MI_U32 idxBatchNum = 0; idxBatchNum < stInvokeParam.u32BatchN; idxBatchNum++)
            {
                memcpy((MI_U8 *)stInvokeParam.astArrayTensors[idxInputNum].ptTensorData[0] + idxBatchNum * IPU_CalcTensorSize(desc.astMI_InputTensorDescs[idxInputNum]), pu8ImageData, datasize);
                if(idxBatchNum == 0)
                {
                    if(dump_input_bin)
                    {
                        FILE* stream_input = fopen("inputtoinvoke.bin","w");
                        int input_size = fwrite((MI_U8 *)stInvokeParam.astArrayTensors[idxInputNum].ptTensorData[0] + idxBatchNum * IPU_CalcTensorSize(desc.astMI_InputTensorDescs[idxInputNum]),sizeof(unsigned char),datasize,stream_input);
                        fclose(stream_input);
                    }
            ...
            MI_SYS_FlushInvCache(stInvokeParam.astArrayTensors[idxInputNum].ptTensorData[0], stInvokeParam.u32BatchN * IPU_CalcTensorSize(desc.astMI_InputTensorDescs[idxInputNum]));
        }

        for (MI_U32 idxOutputNum = 0; idxOutputNum < desc.u32OutputTensorCount; idxOutputNum++)
        {
            if (MI_SUCCESS != IPU_TensorMalloc(&stInvokeParam.astArrayTensors[idxOutputNum + totalInputNumber], IPU_CalcTensorSize(desc.astMI_OutputTensorDescs[idxOutputNum]) * stInvokeParam.u32BatchN))
            {
                IPU_Free_Inputtensor_Memory(desc, stInvokeParam,TRUE);
                delete pu8ImageData;
                IPUDestroyChannel(u32ChannelID);
                MI_IPU_DestroyDevice();
                return -1;
    ...
    {
        //batch_n_buf mode: buffer of each input/output batch is standalone.
        //layout example: u32BatchN = 3; u32InputTensorCount = 2; u32OutputTensorCount = 2;
        ...
        for (MI_U32 idxBatchNum = 0; idxBatchNum < stInvokeParam.u32BatchN; idxBatchNum++)
        {
            for (MI_U32 idxInputNum = 0; idxInputNum < desc.u32InputTensorCount; idxInputNum++)
            {
                ...
                totalInputNumber++;
                memcpy(stInvokeParam.astArrayTensors[idxBatchNum * desc.u32InputTensorCount + idxInputNum].ptTensorData[0],pu8ImageData,datasize);
                if(idxBatchNum == 0)
                {
                    if(dump_input_bin)
                    {
                        FILE* stream_input = fopen("inputtoinvoke.bin","w");
                        int input_size = fwrite(stInvokeParam.astArrayTensors[idxBatchNum * desc.u32InputTensorCount + idxInputNum].ptTensorData[0],sizeof(unsigned char),datasize,stream_input);
                        fclose(stream_input);
                    }

                }
                MI_SYS_FlushInvCache(stInvokeParam.astArrayTensors[idxBatchNum * desc.u32InputTensorCount + idxInputNum].ptTensorData[0], datasize);
            }
        }
        ...
        for (MI_U32 idxBatchNum = 0; idxBatchNum < stInvokeParam.u32BatchN; idxBatchNum++)
        {
            for (MI_U32 idxOutputNum = 0; idxOutputNum < desc.u32OutputTensorCount; idxOutputNum++)
            {
                ...
                {
                    IPU_Free_Inputtensor_Memory(desc, stInvokeParam,FALSE);
                    delete pu8ImageData;
                    IPUDestroyChannel(u32ChannelID);
                    MI_IPU_DestroyDevice();
                    return -1;
    ...
    if (MI_SUCCESS != IPU_VariableBufferMalloc(&phyVariableBuffer, OfflineModelInfo.u32VariableBufferSize))
    {
        ...
        {
            IPU_Free_Inputtensor_Memory(desc, stInvokeParam,TRUE);
            IPU_Free_Outputtensor_Memory(desc, stInvokeParam,totalInputNumber,TRUE);

        }
        else
        {
            IPU_Free_Inputtensor_Memory(desc, stInvokeParam,FALSE);
            IPU_Free_Outputtensor_Memory(desc, stInvokeParam,totalInputNumber,FALSE);
        }
        delete pu8ImageData;
        IPUDestroyChannel(u32ChannelID);
        MI_IPU_DestroyDevice();
    ...
    //when using extern user MMA buffer for variable tensor buffer, need to force IPU core.
    stInvokeParam.u32IpuAffinity = 0x1; //bind to IPU0
    ...
    for (int i=0;i<times;i++ )
    {
        ...
        if(bBatchOneBuf)
        {
            if(MI_SUCCESS!=MI_IPU_Invoke2Custom(u32ChannelID, &stInvokeParam, &stRuntimeInfo))
            {
    ...
    //13. Show classify result of first output of all batches
    IPU_PrintOutputXOR(desc, stInvokeParam, bBatchOneBuf);
    ...
    for (MI_U32 idxBatchNum = 0; idxBatchNum < stInvokeParam.u32BatchN; idxBatchNum++)
    {
        ...
        if(bBatchOneBuf)
        {
            pfData = (float *)stInvokeParam.astArrayTensors[totalInputNumber].ptTensorData[0] + s32ClassCount*idxBatchNum;
        }
        else
        {
            pfData = (float *)stInvokeParam.astArrayTensors[totalInputNumber + idxBatchNum*desc.u32OutputTensorCount].ptTensorData[0];
        }
