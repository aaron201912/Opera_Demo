...
{
    // Sensor_Attr_t sclAttr;
    // if(get_sensor_attr(buf_obj->sensorIdx, &sclAttr) != 0)
    // {
    //     printf("get_sensor_attr fail \n");
    //     return -1;
    // }
    // buf_obj->chn_port_info.eModId = E_MI_MODULE_ID_SCL;
    // buf_obj->chn_port_info.u32DevId = sclAttr.u32SclDevId;
    // buf_obj->chn_port_info.u32ChnId = sclAttr.u32SclChnId;
    // buf_obj->chn_port_info.u32PortId = sclAttr.u32SclOutPortId;
...
int sstar_drm_getattr(buffer_object_t *buf)
{
    ...
    //printf("buf->width=%d buf->height=%d video_info_plane_id=%d buf->crtc_id %d buf->conn_id %d\n",buf->width,buf->height,buf->vdec_info.plane_id,buf->crtc_id,buf->conn_id);
	_g_res = buf->_g_res;
	_g_conn = buf->_g_conn;
...
int drm_realtime_atomic_commit(buffer_object_t *bobj) {
    int ret;
    drmModeAtomicReqPtr req;
    ...
    if(!bobj->drm_commited)
    {
		...
		if (bobj->connector_type == DRM_MODE_CONNECTOR_DPI) 
		{
		...
		}else if(bobj->connector_type == DRM_MODE_CONNECTOR_DSI)
		{
			drmModeAtomicAddProperty(req, bobj->vdec_info.plane_id, prop_ids->CRTC_X, 0);
        	drmModeAtomicAddProperty(req, bobj->vdec_info.plane_id, prop_ids->CRTC_Y, 0);
        ...
        {
            printf("realtime mode only support MOPS \n");
            return -1;
        }
        bobj->drm_commited = 1;
        ...
        if(ret != 0)
        {
            printf("Waring: drmModeAtomicCommit failed ret=%d fd=%d plane_id=%d\n",ret , bobj->fd, bobj->vdec_info.plane_id);
            //return -1;
        }
        drmModeAtomicFree(req);
