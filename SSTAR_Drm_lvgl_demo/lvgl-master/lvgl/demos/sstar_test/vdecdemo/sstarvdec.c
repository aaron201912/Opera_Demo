...
void * sstar_video_thread(void* arg)
{
	...
	MI_U8 sps_pps_flag = 0;
	MI_U8 *pu8Sps_Pps_Buf = NULL;
    ...
    pu8Sps_Pps_Buf = (MI_U8 *)malloc(MAX_ONE_FRM_SIZE);
    if (!pu8Sps_Pps_Buf) {
        printf("ST_VdecSendStream: malloc pu8Sps_Pps_Buf error\n");
        return NULL;
    }
    ...
    while (!bVdecExit)
    {
		...
		if(sps_pps_flag == 0)
		{
			...
			stVdecStream.u32Len = 0;
		}
		else
		{
			stVdecStream.pu8Addr = (MI_U8 *)pu8Sps_Pps_Buf;
		}
        
        if(	(0 == *(pstNalu->buf) && 0 == *(pstNalu->buf+1) && 0 == *(pstNalu->buf+2) && 1 == *(pstNalu->buf+3) && (8 == (*(pstNalu->buf+4) & 0x0F) || 7 == (*(pstNalu->buf+4) & 0x0F)))
			|| (0 == *(pstNalu->buf) && 0 == *(pstNalu->buf+1) && 1 == *(pstNalu->buf+2) && (8 == (*(pstNalu->buf+3) & 0x0F) || 7 == (*(pstNalu->buf+3) & 0x0F))))
        {
			memcpy((void*)(pu8Sps_Pps_Buf + stVdecStream.u32Len), pstNalu->buf, pstNalu->len);
...
int sstar_vdec_init(vdec_info_t* vdec_info, void *video_file)
{
    ...
    stChnAttr.u32PicWidth                    = ALIGN_BACK(vdec_info->v_src_width , 32);
    stChnAttr.u32PicHeight                   = ALIGN_BACK(vdec_info->v_src_height, 32);
    ...
    stOutputPortAttr.u16Width  = ALIGN_BACK(vdec_info->v_out_width, 32);
    stOutputPortAttr.u16Height = ALIGN_BACK(vdec_info->v_out_height, 32);
    ...
    if(video_file)
    {
        strcpy(file_path,video_file);
    }

    pthread_create(&_g_tid_video, NULL, sstar_video_thread, (void *)file_path);
