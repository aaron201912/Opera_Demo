...
int AppTurnOffPcm(snd_pcm_t* pst_handle)
{
	int err = -1;
	err = snd_pcm_drain(pst_handle);
	err = snd_pcm_close(pst_handle);
    err = snd_pcm_hw_free(pst_handle);
    Ao_dac_attach("0");
	return err;
}
...
void *mp3DecodeProc(void *pParam)
{
	...
	printf("@@@@@@@@@@mp3DecodeProc g_bPcmDevInit %d \n", g_bPcmDevInit);
    ...
    while(g_bPlayThreadRun)    //1.2 循环读取mp3文件中的数据帧
    {
        ...
        if(g_bPcmDevInit == false)
        {
	        ...
	        stAlsaInit.sample_rate = frame->sample_rate;
	        stAlsaInit.chanel_size = frame->channels;
	        stAlsaInit.buffer_frames = 4608;
	        stAlsaInit.access_type = SND_PCM_ACCESS_RW_INTERLEAVED;
	        stAlsaInit.pcm_format = SND_PCM_FORMAT_S16_LE;
	        stAlsaInit.frame_size = stAlsaInit.sample_size * stAlsaInit.chanel_size;
	        stAlsaInit.period_frames = stAlsaInit.buffer_frames/stAlsaInit.frame_size;
	        stAlsaInit.period_buff_size = stAlsaInit.period_frames * stAlsaInit.frame_size;
            AppPcmInit(&stAlsaInit, "default");
        }
        AppPlayDataStream(stAlsaInit.handle, (char *)is->audio_buf, wavDataLen);
...
int init_ffplayer(AudioState* is, char* filepath)
{
	...
	printf("@@@ alan test videoindex=%d, sndindex=%d \n", is->videoindex, is->sndindex);
    if(is->sndindex != -1)
    {
...
#endif
AudioState* is;
int lv_demo_init_mp3_ffplayer()
{
	int ret = 0;
	is = (AudioState*) av_mallocz(sizeof(AudioState));
	if( (ret=init_ffplayer(is, "/customer/res/layout_test.mp3") != 0))
    {
        printf("init_ffmpeg error");
        goto uninstall;
    }
...
int lv_demo_deinit_mp3_ffplayer()
{
	deinit_ffplayer(is);
    AppTurnOffPcm(stAlsaInit.handle);
    av_free(is);
