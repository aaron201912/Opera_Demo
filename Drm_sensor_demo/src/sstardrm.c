...
void sstar_drm_init(buffer_object_t *buf)
{
	...
	buf->plane_id = get_plane_with_format(buf->fd, buf->format);//plane_res->planes[1];
	buf->vdec_info.plane_id = get_plane_with_format(buf->fd, buf->vdec_info.format);//plane_res->planes[1];
...
static int drm_atomic_commit(buffer_object_t *bobj ,dma_info_t* dma_info) {
    ...
    if(!bobj->drm_commited)
    {
        ...
        drmModeAtomicAddProperty(req, bobj->vdec_info.plane_id, prop_ids->SRC_X, 0);
        drmModeAtomicAddProperty(req, bobj->vdec_info.plane_id, prop_ids->SRC_Y, 0);
        ...
        drmModeAtomicAddProperty(req, bobj->crtc_id, prop_ids->FENCE_ID, (int)&bobj->vdec_info.out_fence);//use this flag,must be close out_fence
    ...
    else
    {
        ...
        new_plane.fence = (int)&dma_info->out_fence;
